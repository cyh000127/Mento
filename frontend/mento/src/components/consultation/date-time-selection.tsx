...
export function DateTimeSelection({ selectedDate, selectedTime, selectedCategory, onDateSelect, onTimeSelect, onNext, onPrev, canProceed }: DateTimeSelectionProps) {
  ...
  const getSlotForTime = (time: string) => {
    ...
    return selectedDateSlots.find((candidate) => normalizeScheduledTime(candidate.scheduledTime) === time) ?? null;
  };

  const isTimeUnavailable = (time: string) => {
    if (!selectedDateSlots) return true;
    const slot = getSlotForTime(time);
    return !slot || slot.status === "CLOSED" || slot.availableCapacity <= 0;
  };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-2xl font-bold text-text-primary">상담 받을 날짜와 시간을 선택해 주세요</h1>
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-sm">
          ...
          <div className="mb-6 flex items-center justify-between">
            <button type="button" onClick={prevMonth} className="rounded-lg p-2 text-text-secondary transition-colors hover:bg-muted hover:text-text-primary">
              <ChevronLeft className="h-5 w-5" />
            </button>
            <h2 className="text-lg font-bold text-text-primary">
              {currentYear}.{currentMonth + 1}
            </h2>
            <button type="button" onClick={nextMonth} className="rounded-lg p-2 text-text-secondary transition-colors hover:bg-muted hover:text-text-primary">
          ...
          <div className="mb-2 grid grid-cols-7 gap-1">
            {DAYS_KO.map((day, i) => (
              <div key={day} className={`py-2 text-center text-sm font-medium ${i === 0 ? "text-red-400" : i === 6 ? "text-blue-400" : "text-text-secondary"}`}>
                {day}
              </div>
            ))}
          </div>
            ...
            {daysInMonth.map((date, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {todayDate && inCurrentMonth && <span className={`text-[10px] ${selected ? "text-dark-bg/80" : "text-primary-500"}`}>오늘</span>}
                  {isBooked && inCurrentMonth && !todayDate && <span className="text-[10px] text-text-secondary/60">마감</span>}
                </button>
              );
            })}
          </div>
