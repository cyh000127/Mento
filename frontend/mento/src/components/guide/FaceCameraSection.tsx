...
export function FaceCameraSection({
  selectedArea,
  onAreaSelect,
  ...
  return (
    <div className="flex flex-col items-center gap-6">
        ...
        <div className={`absolute inset-0 flex flex-col items-center justify-center p-6 bg-background ${!cameraOn || isLoading ? 'flex' : 'hidden'}`}>
          <div className="mb-6 h-24 w-24 rounded-full bg-muted flex items-center justify-center">
            {isLoading ? (
              ...
              <CameraOff className="h-12 w-12 text-text-secondary" />
            )}
          </div>
          <p className="text-center text-sm text-text-secondary mb-6 max-w-[280px]">
            {isLoading
              ? "카메라를 켜는 중..."
              : "카메라를 켜고 얼굴에 직접 제품을 적용해보세요"}
          </p>
          <button
            onClick={toggleCamera}
            disabled={isLoading}
                ...
                className={`
                  ...
                  ${
                    isSelected
                      ...
                      : "bg-background border-2 border-border text-text-primary hover:border-primary-300 hover:bg-primary-100/30"
                  }
                `}
      ...
      {/* Additional Info */}
      <div className="w-full max-w-[400px] p-4 bg-pastel-blue-100 rounded-lg border border-pastel-blue-200">
        <p className="text-xs text-text-secondary text-center">
          <span className="font-semibold text-primary-500">💡 Tip:</span>{" "}
          카메라를 얼굴 정면에서 약 30cm 거리에 두고 사용하세요
        </p>
      </div>
    </div>
  )
