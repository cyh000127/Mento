...
export async function createConsultationSession(
...
): Promise<ConsultationSessionData> {
  ...
  try {
    const response = await api.post<ConsultationSessionResponse>(
      `/reservations/${reservationId}/sessions`
    )
    ...
    if (!response.data.success || !response.data.data) {
      throw new Error(
        response.data.error?.message || "상담 세션 생성에 실패했습니다."
      )
    }
